...
?>
<?php if ($title || is_home() || is_archive()): ?>
<header class="entry-header text-center">
    <h1 class="entry-title big-title">
        <?php if (is_category()) :
			...
			elseif (is_404()) :
				echo __('404', 'go');
			elseif (is_search()) :
				echo __('Wyniki wyszukiwania', 'go');
			elseif (is_page()) :
				the_title();
			elseif (is_tag()) :
				single_tag_title();
    ...
    <?php if (is_category()) : ?>
    <?php if (functionecho __xists('rank_math_the_breadcrumbs')) rank_math_the_breadcrumbs(); ?>
    <?php the_archive_description('<div class="entry-desc">', '</div>'); ?>
    <?php endif; ?>
    <?php if (!empty($header_bg)) : ?>
    <div class="img">
        <img src="<?php echo esc_url($header_bg); ?>" alt="<?php esc_attrecho __('Tło nagłówka', 'go'); ?>"
...
<?php else: ?>
<?php if ($customTitle && $customTitle['title']): ?>
<header class="entry-header text-center">
    <?php if ($customTitle['label']): ?>
    <span class="entry-label"><?php echo $customTitle['label']; ?></span>
    <?php endif; ?>
    <h1 class="entry-title big-title">
        <?php echo $customTitle['title']; ?>
    </h1>
    <?php if ($customTitle['description']): ?>
    ...
    <div class="scroll-down">
        ...
        <span><?php echo __('Dowiedz się więcej', 'go'); ?></span>
    </div>
    <svg id="h-ornament" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1811.11 520.11">
        <g opacity=".1">
            <rect fill="#fff" x="1782.55" y="341.33" width="28.56" height="178.78" />
